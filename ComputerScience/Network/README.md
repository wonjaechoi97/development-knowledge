...
>>>>- 클라이언트는 HTTP request를 보내고 서버에선 HTTP response를 반환
>>>>- 과거 HTTP/1.0 버전에서는 요청을 보낼 때마다 연결했다 끊는 작업을 반복
>>>>>- 현재는 HTTP/2 와 http/1.1 버전에서는 Keepalive 기능을 사용하여 연결을 한 번 수립하면 데이터 교환을 마칠 때까지 유지하고, 데이터 교환을 모두 끝내면 연결을 끊는 구조
...
>>>- DNS 서버는 이름 해석을 통해 도메인 이름을 IP 주소로 변환
>>>>- DNS 서비스가 동작하는 컴퓨터를 DNS 서버라고 부름
>>>>- 도메인 이름: www.naver.com 과 같이 컴퓨터나 네트워크를 식별하기 위해 붙여진 이름
>>>>- 호스트 이름(서버 이름): 도메인 이름 앞에 있는 www
>>>- DNS 서버는 전 세계에 흩어져 있고 모두 계층적으로 연결되어 연계하면서 동작함
...
>>>>- 포트번호로 110번 사용
>>- 메일 송수신 과정
>>>1. 컴퓨터1: 메일 서버 1의 25번 포트로 SMTP 프로토콜로 메일을 보냄
>>>2. 메일 서버1: 받은 메일을 메일 서버 2의 25번 포트로 SMTP 프로토콜로 메일을 보냄
>>>3. 컴퓨터2 와 메일 서버 사이에서 110번 포트를 통해 POP3 프로토콜로 메일을 가져가거나 가져옴
>- SMTP에 의한 메일 송신과 메일 전송
>>1. 컴퓨터가 메일 서버1로 세션 시작을 통지
>>2. 송신자의 메일 주소를 통지
>>3. 목적지 메일 주소를 통지
>>4. 메일 본문 전송을 통지
>>5. 메일 본문을 송신
...
>>- 메일 서버2는 POP3 프로토콜로 메일 서버2의 메일 박스에서 메일을 가져와 컴퓨터2로 전송
>>- 메일을 수신할 때는 사용자 이름과 비밀번호를 이용한 사용자 인증이 필요
>>1. 컴퓨터2가 메일 서버2로 세션 시작을 통지
>>2. 수신자의 사용자 이름을 통지
>>3. 수신자의 비밀번호를 통지
>>4. 메일을 확인
>>5. 메일의 전송을 요청
