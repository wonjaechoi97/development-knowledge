...
>>>- [모니터 (Monitors)](#모니터-monitors)
>>>- [라이브니스 (Liveness)](#라이브니스-liveness)
>>- [교착 상태](#교착-상태)
>>>- [시스템 모델](#시스템-모델)
>>>- [다중 스레드 응용에서의 교착 상태](#다중-스레드-응용에서의-교착-상태)
>>>- [교착 상태 특성](#교착-상태-특성)
>>>- [교착 상태 처리 방법](#교착-상태-처리-방법)
>>>- [교착 상태 예방](#교착-상태-예방)
...
>### 세마포 (Semaphores)
>- 기초 개념
>>- 세마포 S 는 정수 변수로서, 초기화를 제외한 wait()와 signal() 두 개의 표준 원자적 연산으로만 접근할 수 있음
>>```c
>>wait(S) {
>>  while(S<=0)
>>    ; /* busy wait */
...
>>}
>>```
>>- wait() 과 signal() 연산 시 세마포의 정수 값을 변경하는 연산은 반드시 원자적으로 수행되어야 함
>>>- 한 스레드가 세마포 값을 변경하면, 다른 어떤 스레드도 동시에 동일한 세마포 값을 변경할 수 없어야 함
>>- wait(S)의 경우, S의 정수 값을 검사하는 작업 (S<=0) 과 그에 따라 실행될 수 있는 변경 (S--) 하는 작업 또한 인터럽트 되지 않고 실행되어야 함
>- 세마포 사용법
>>- 카운팅 세마포
>>>- 세마포의 값이 제한 없는 영역 (domain) 을 가짐
>>>- 유한한 개수를 가진 자원에 대한 접근을 제어하는 데 사용될 수 있으며, 세마포는 가용한 자원의 개수로 초기화됨
>>>- 각 자원을 사용하려는 프로세스는 세마포의 wait() 연산을 수행하며 세마포의 값은 감소됨
>>>- 프로세스가 자원을 방출할 떄 signal() 연산을 수행하며 세마포의 값은 증가함
>>>- 세마포의 값이 0일 경우 모든 자원이 사용 중임을 나타내며, 이후 자원을 사용하려는 프로세스는 세마포 값이 0보다 커질 떄까지 봉쇄됨
>>- 이진 세마포
>>>- 세마포의 값이 0과 1의 값만 가짐
>>>- mutex lock 과 유사하게 동작함
>- 세마포 구현
>>- 바쁜 대기
>>>- mutex lock 에서 바쁜 대기로 인한 CPU 낭비 문제가 발생하였는데, 세마포 연산 wait() 과 signal() 의 정의 역시 바쁜 대기가 발생함
>>>- 바쁜 대기를 극복하기 위해 정의를 변경함
>>>>- 프로세스가 wait() 연산을 실행하고 세마포 값이 양수가 아닌 것을 발견하면, 프로세스는 반드시 대기
>>>>- 바쁜 대기 대신 프로세스는 자신을 일시 중지 시킴
>>>>- 일시 중지 연산은 프로세스를 세마포에 연관된 대기 큐에 넣고, 프로세스의 상태를 대기 상태로 전환하며 이후 제어가 CPU 스케줄러로 넘어감
>>>>- 세마포 S를 대기하면서 일시 중지된 프로세스는 다른 프로세스가 signal() 연산을 실행하면 재시작되어야 함
>>>>- 프로세스는 wakeup() 연산에 의해 재시작되며, wakeup() 연산은 프로세스의 상태를 대기상태에서 준비 완료 상태로 변경하며, 이후 프로세스는 준비 완료 큐에 넣어짐
>>>>```c
>>>>typedef struct {
>>>>  int value;
>>>>  struct process *list;
...
>>>>}
>>>>```
>>>>- 바쁜 대기를 하는 세마포의 고전적 정의에서는 세마포의 값은 음수를 가질 수 없으나, 정의를 변경하면 음수 값을 가질 수 있음
>>>>- 세마포 값이 음수일 때, 그 절대값은 세마포를 대기하고 있는 프로세스들의 수와 같음
...
>### 모니터 (Monitors)
>- mutex lock 과 세마포의 시간적 오류
>>- 세마포에서 각 프로세스는 임계구역에 진입 전 wait()를 실행하고 임계구역을 나올 때 signal()을 실행하하는데, 만약 이 순서가 지켜지지 않으면 두 프로세스가 동시에 임계구역 안에 있을 수 있음
>>- 오직 하나의 프로세스라도 잘못 행동하면 발생하는 여러 문제점이 존재하는데, 순수한 프로그래밍 오류 또는 비협조적인 프로그래머에 의해 야기됨
>>>- 세마포에 대한 wait()와 signal() 연산의 순서가 바뀐 경우
>>>>- 여러 프로세스가 동시에 임계구역 안에서 실행될 수 있어 상호 배제 요구 조건 위반
>>>- signal()을 써야할 곳에 wait()를 작성한 경우
>>>>- 세마포를 사용할 수 없으므로 프로세스는 두 번쨰 wait() 호출에서 영구적으로 봉쇄됨
>>>- 프로세스가 wait()나 signal 또는 둘 다 빠뜨린 경우
>>>>- 상호 배제 요구 조건을 위반하든지 프로세스는 영원히 봉쇄됨
>>- 다양한 유형의 오류를 처리하기 위한 한 가지 전략으로 간단한 동기화 도구를 통합하여 모니터와 같은 고급 언어 구조물을 제공함
>- 모니터 사용법
>>- 추상화된 데이터 형 (abstract data type, ADT)
>>>- 데이터와 이 데이터를 조작하는 함수들과의 집합을 하나의 단위로 묶어 보호함
>>>- 함수의 구현은 ADT의 특정한 구현과는 독립적
>>>- 모니터 형은 모니터 내부에서 상호 배제가 보장되는 프로그래머가 정의한 일련의 연산자 집합을 포함하는 ADT임
>>- 모니터 구조물
>>>- 모니터 구조물은 모니터 안에 항상 하나의 프로세스만이 활성화되도록 보장해주기 때문에 프로그래머들은 이와 같은 동기화 제약 조건을 명시적으로 코딩해야 할 필요가 없음
>>>- 모니터 구조물은 어떤 동기화 기법을 모델링 하는 데 충분한 능력을 제공하지 않기 때문에 부가적인 동기화 기법을 정의해야 함
>>- condition
>>>- 동기화 기법을 작성할 필요가 있는 프로그래머는 하나 이상의 condition 변수를 정의할 수 있음
>>>- condition 형 번수에 호출될 수 있는 연산은 wait()와 signal()
>>>```c
>>>condition x, y;
>>>x.wait();
>>>x.signal();
>>>```
>>>- wait 연산을 호출한 프로세스는 다른 프로세스가 signal을 호출할 때까지 일시 중지 되어야 함
>>>- signal() 연산은 정확히 하나의 일시 중지 프로세스를 재개하며, 일시 중지된 프로세스가 없으면 연산은 아무런 효과가 없음
>>- 모니터 유의사항
>>>- 일시 중지된 스레드 Q가 실행을 재개하도록 허용된다면 signal을 보낸 스레드 P는 반드시 대기해야 하며, 그렇지 않으면 P와 Q는 모니터 안에서 동시에 활성화됨
>>>- 두 프로세스는 개념적으로 그들의 실행을 계속할 수 있으며, 이로 인해 두 가지 가능성이 존재함
>>>>- Signal and wait : P는 Q가 모니터를 떠날 떄까지 기다리거나 또는 다른 조건을 기다림
>>>>- Signal and continue : Q는 P가 모니터를 떠날 때까지 기다리거나 또는 다른 조건을 기다림
>- 세마포를 이용한 모니터의 구현
>>- 기본 구조
>>>- 각 모니터마다 mutex라는 이진 세마포가 정의되고 초기 값은 1
>>>- 프로세스는 모니터로 들어가기 전에 wait(mutex)를 실행하고 모니터를 나온 후에 signal(mutex)을 실행해야 함
>>>- 모니터 구현 시 signal-and-wait 기법을 사용하며 Signaling 프로세스는 실행 재개되는 프로세스가 모니터를 떠나든지 아니면 wait() 할 때까지 그 자신이 다시 기다려야 하므로 next라는 이진 세마포가 추가로 필요하게 되고 0으로 초기화됨
>>>- Signaling 프로세스는 자신을 중단시키기 위해 next를 사용할 수 있음
>>>- 정수형 변수 next_count도 next에서 일시 중지 되는 프로세스의 개수를 세기 위해 제공됨
>>>```c
>>>wait(mutex);
>>>  ...
>>>  body of F
...
>>>  signal(mutex);
>>>```
>>>>- 이처럼 하면 모니터 안에서의 상호 배제가 보장됨
>>- 조건 변수를 세마포로 구현
>>>- 각 조건 x마다 x_sem 이라는 이진 세마포와 x_count 라는 정수형 변수를 도입하고 둘 다 초기 값은 0
>>>```c
>>>// x.wait()
>>>x_count++;
>>>if(next_count > 0)
...
>>>```
>- 모니터 내에서 프로세스 수행 재개
>>- conditional-wait 구조물
>>>- 형태 : ```x.wait(c)```
>>>- c는 정수 수식 (expression) 이고 wait() 연산이 호출될 때 값이 계산 됨
>>>- c의 값은 우선순위 번호로 일시 중지 되는 프로세스의 이름과 함께 저장됨
>>>- x.signal()이 수행되면 가장 작은 우선순위 번호를 가진 프로세스가 다음번에 수행 재개 됨
>>>```c
...
>>>}
>>>
>>>// ResourceAllocator 의 자원을 액세스 하려는 프로세스는 이 순서를 따라야 함
>>>R.acquire(t);
>>>  ...
>>>  access the resource;
>>>  ...
>>>R.release();
>>>```
>>>- 모니터의 개념은 위에서 예시한 순서가 그대로 지켜지는 것을 보장해주지 않아 다음과 같은 문제가 발생할 수 있음
...
>>>- 이러한 접근 제어 문제는 17장에 설명된 추가적인 기법을 사용해야만 해결할 수 있음
...
>### 라이브니스 (Liveness)
>- 개요
>>- 임계구역에 대한 액세스를 조정하기 위해 동기화 도구를 사용하는 것의 한 가지 결과는 임계구역에 들어가려고 시도하는 프로세스가 무기한 대기할 가능성이 있다는 것
>>- 라이브니스
>>>- 프로세스가 실행 수명주기 동안 진행되는 것을 보장하기 위해 시스템이 충족해야 하는 일련의 속성
>>- 무기한 대기하는 프로세스는 "라이브니스 실폐"의 한 가지 예시
>>- 다양한 형태의 라이브니스 실패가 존재하나 모두 성능과 응답성이 나쁜 것이 특징임
>- 교착 상태 (Deadlock)
>>- 대기 큐를 가진 세마포의 구현에서 두 개 이상의 프로세스들이 대기 중인 프로세스들 중 하나에 의해서만 야기되는 이벤트 (signal() 연산의 실행) 를 무한정 기다리는 상황이 발생했을 때의 상태
>>- 두 개의 프로세스 P0, P1 로 구성되고 1로 지정된 세마포 S, Q에 접근하는 시스템에서의 예시
>>>1. P0 가 wait(S)를 실행하고, P1 이 wait(Q)를 실행한다고 가정
>>>2. P0 가 wait(Q)를 실행할 때, P0은 P1이 signal(Q)를 실행할 때까지 대기해야함
>>>3. 마찬가지로 P1이 wait(S)를 실행할 때, P0가 signal(S)를 실행할 때까지 대기해야함
>>>- signal() 연산들은 실행될 수 없으므로 P0 과 P1은 교착상태가 됨
>- 우선순위 역전
>>- 문제
>>>- 높은 우선순위 프로세스가 현재 낮은 우선순위 프로세스 또는 연속된 낮은 우선순위 프로세스들에 의해 접근되고 있는 커널 데이터를 읽거나 변경할 필요가 있을 때 스케줄링의 어려움이 생기게 됨
>>>- 통상적으로 커널 데이터는 락에 의해 보호되기 때문에 낮은 우선순위 프로세스가 자원의 사용을 마칠 때까지 높은 우선순위 프로세스가 기다려야 함
>>>- 낮은 우선순위 프로세스가 또 다른 높은 우선순위 프로세스에 의해 선점되는 경우에 상황은 더욱 복잡해짐
>>- 우선순위 상속 프로토콜을 구현하여 해결함
>>>- 더 높은 우선순위 프로세스가 필요로 하는 자원에 접근하는 모든 프로세스는 문제가 된 자원의 사용이 끝날 때까지 더 높은 우선순위를 상속받음
>>>- 자원 사용이 끝나면 원래 우선순위로 되돌아감
...
>### 시스템 모델
>- 개요
>>- 시스템은 경쟁하는 스레드들 사이에 분배되어야 할 유한한 수의 자원들로 구성됨
>>- 자원은 다수의 유형으로 (또는 클래스) 분할되며, 각각은 동등한 다수의 인스턴스로 구성됨
>>>- 자원의 유형으로 CPU 주기, 파일, 입/출력 장치 (네트워크 인터페이스, DVD 드라이브 등) 등이 있음
>>>- 한 시스템이 4개의 CPU를 가진다면, 자원 유형 CPU는 4개의 인스턴스를 가짐
>>- 한 스레드가 어떤 자원 유형의 한 인스턴스를 요청하면, 동일 유형 자원의 임의의 인스턴스를 할당함으로써 요청이 충족됨
>- 교착 상태
>>- Mutex Lock 과 세마포 등의 다양한 동기화 도구도 시스템 자원이며, 현재 컴퓨터 시스템에서는 가장 일반적인 교착 상태의 발원지임
>>- 락은 일반적으로 특정 자료구조와 연관되어 있음
>>>- 하나의 락은 큐에 대한 액세스를 보호하고 다른 하나는 연결 리스트에 대한 액세스를 보호하는 데 사용될 수 있으므로 락의 각 인스턴스에는 일반적으로 고유한 자원 클래스가 할당됨
>>- 스레드는 다른 프로세스의 자원을 사용할 수 있으며 이러한 자원 사용으로 인해 교착 상태가 발생할 수 있음
>- 자원 사용
>>- 스레드는 자원을 사용하기 전에 반드시 요청해야 하고, 사용 후에는 반드시 방출해야 함
>>- 스레드는 지정된 태스크를 수행하기 위해 필요한 만큼의 자원을 요청할 수 있으며, 요청된 자원의 수는 시스템에서 사용 가능한 전체 자원의 수를 초과하면 안됨
>>- 정상적인 작동 모드에서 프로세스의 자원 사용 순서
>>>1. 요청 : 스레드는 자원을 요청하며, 요청이 즉시 허용되지 않으면 (mutex lock을 다른 스레드가 가지고 있는 경우등) 요청 스레드는 자원을 얻을 때까지 대기
>>>2. 사용 : 스레드는 자원에 대해 작업을 수행 (자원이 mutex lock 이라면 스레드는 자신의 임계구역에 접근할 수 있음)
>>>3. 방출 : 스레드가 자원을 방출
>>- 요청과 방출
>>>- 자원의 요청과 방출은 장치의 request(), release(), 파일의 open(), close(), 메모리의 allocate(), free() 등의 시스템콜임
>>>- 세마포의 획득과 방출은 세마포에 대한 wait(), signal()
>>>- mutex lock의 획득과 방출은 acquire(), release()
>>- 운영체제의 역할
>>>- 스레드가 커널이 관리하는 자원을 사용할 때마다 매번 운영체제는 스레드가 자원을 요청했는지와 자원을 할당받았는지를 확인함
>>>- 시스템 테이블이 각 자원이 가용 상태인지 할당되었는지 기록하며, 각 할당된 자원에 대해서 어느 스레드에 할당되었는지 기록함
>>>- 어떤 스레드가 현재 다른 스레드에 할당된 자원을 요청하면 그 자원을 기다리는 스레드 큐에 입력될 수 있음
>- 정리
>>- 한 스레드 집합 내의 모든 스레드가 그 집합 내의 다른 스레드에 의해서만 발생될 수 있는 이벤트를 기다린다면, 그 스레드 집합은 교착 상태에 있음
>>- 주된 이벤트는 자원의 획득과 방출이지만, 네트워크 인터페이스로부터 읽거나 IPC (프로세스간 통신) 기능 등을 포함하여 다른 유형의 이벤트도 교착 상태를 만들어낼 수 있음
>>- 다중 스레드 응용 개발자는 반드시 교착 상태의 가능성을 염두에 두어야함
>>- 락킹 도구들은 경쟁 조건을 회피하도록 설계되었으나 이러한 도구를 사용할 때, 개발자는 락이 획득되고 방출되는 방식에 대해 교착 상태가 발생하지 않도록 주의를 기울여야 함
...
>### 교착 상태 특성
>- 필요조건
>>- 교착 상태는 한 시스템에 네 가지 조건이 동시에 성립될 때 발생할 수 있음
>>1. 상호 배제 (mutual exclusion)
>>>- 최소한 하나의 자원이 비공유 모드로 점유되어야함
>>>- 비공유 모드에서는 한 번에 한 스레드만이 그 자원을 사용할 수 있음
>>>- 다른 스레드가 그 자원을 요청하면, 요청 스레드는 자원이 방출될 때까지 반드시 지연되어야 함
>>2. 점유하며 대기 (hold-and-wait)
>>>- 스레드는 최소한 하나의 자원을 점유한 채, 현재 다른 스레드에 의해 점유된 자원을 추가로 얻기 위해 반드시 대기해야 함
>>3. 비선점 (no preemption)
>>>- 자원들을 선점할 수 없어야 함
>>>- 즉, 자원이 강제적으로 방출될 수 없고, 점유하고 있는 스레드가 태스크를 종료한 후 그 스레드에 의해 자발적으로만 방출될 수 있음
>>4. 순환 대기 (circular wait)
>>>-  대기하고 있는 스레드의 집합에서 꼬리에 꼬리를 물며 대기함
...
>### 교착 상태 처리 방법
>- 원칙적으로 교착 상태 문제를 처리하는 세 가지 다른 방법
>>1. 문제를 무시하고, 교착 상태가 시스템에서 절대 발생하지 않는 척함
>>2. 시스템이 결코 교착 상태가 되지 않도록 보장하기 위하여 교착 상태를 예방하거나 회피하는 프로토콜을 사용함
>>3. 시스템이 교착 상태가 되도록 허용한 다음에 복구시키는 방법
>- 교착 상태가 발생하지 않도록 하기 위한 시스템
>>- 교착 상태 예방 (prevention)
>>>- 교착 상태 필요조건 중 적어도 하나가 성립하지 않도록 보장하는 일련의 방법
>>- 교착 상태 회피 (avoidence)
>>>- 스레드가 평생 요구하고 사용할 자원에 대한 부가적인 정보를 미리 제공받아 운영체제는 각 요청을 위해 그 스레드가 기다려야 할지 않을지를 결정할 수 있음
>- 교착 상태 탐지 및 복구
>>- 시스템이 교착 상태 예빵 혹은 회피 알고리즘을 사용하지 않으면 교착 상태가 발생할 수 있음
>>- 시스템은 교착 상태가 발생했는지 결정하기 위해 시스템의 상태를 조사하는 알고리즘과 교착 상태로부터 복구하기 위한 알고리즘을 제공할 수 있음
>>- 교착 상태를 탐지하고 복구하는 알고리즘이 없다면 탐지되지 않은 교착 상태는 수행할 수 없는 스레드에 의해 자원이 점유되어 있고 많은 스레드들이 자원을 요청함에 따라 교착 상태로 진입하기 때문에 시스템 성능을 저하하게 되며, 시스템은 작동을 정지하고 수작업으로 다시 시작할 필요가 생김
