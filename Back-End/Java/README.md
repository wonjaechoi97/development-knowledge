...
>>- [변경 가능성을 최소화하기](#변경-가능성을-최소화하기)
>>- [상속(클래스 간 상속)보다는 컴포지션을 사용하기](#상속클래스-간-상속보다는-컴포지션을-사용하기)
...
>### 변경 가능성을 최소화하기
>- 불변 객체
>>- 근본적으로 스레드 안전하여 따로 동기화할 필요가 없으며 안심하고 공유할 수 있음
...
>### 상속(클래스 간 상속)보다는 컴포지션을 사용하기
>- 상속은 캡슐화를 꺠뜨리는 문제가 있음
>>- 상위 클래스와 하위 클래스가 순수한 is-a 관계일 떄만 사용해야함
>>>- 하위 클래스의 패키지가 상위 클래스와 다르고, 상위 클래스가 확장을 고려해 설계되지 않았다면 문제가 될 수 있음
* 재정리 필요
